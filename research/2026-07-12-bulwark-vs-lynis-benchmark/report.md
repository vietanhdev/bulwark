...
(no Docker), `rkhunter --configfile <conf> --check` and separately `--list tests`, `--version`,
...
- **rkhunter 1.4.6** (`apt install rkhunter`, `rkhunter --check --sk --nocolors --logfile
  <path>`): checked **461 rootkit signatures** and **116 file properties** in 49 seconds. Real
...
- **Native GUI + continuous monitoring**: Lynis is CLI/cron-only by design (confirmed directly
  ...
  Bulwark's file-watcher caught in earlier dogfooding this session re-triggers on a sensitive
  file edit in seconds, not at the next cron interval.
