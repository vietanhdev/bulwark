...
**Redaction destroyed `.env` secrets (data-loss, fixed).** A live provider key in a project `.env` was reported with `redactable=true`, so `bulwarkctl ai redact --apply` (and the GUI redact) rewrote the real value to `[bulwark:redacted-secret]` — breaking the project *and* destroying the only copy of the key in that file. A `.env` is the textbook functional secrets file: something reads the value back, so rewriting it in place is pure damage. Fixed in three layers, all load-bearing: (1) `kind_allows_redaction` in `ai_scan/mod.rs` makes only genuine *leak surfaces* redactable — `Transcript` and `Instructions`, records/context that no tool reads for their value — while `DotEnv`/`McpConfig`/`Settings`/`Tasks`/`CodexConfig`/`Credential` never are; (2) the GUI's `redactable_files()` allowlist filters on `f.redactable`, not merely "the scan flagged this file"; (3) `redact_one` refuses any `.env`/`.env.*` by name as a kind-agnostic backstop, even if an upstream bug ever selects one. Secrets are still *reported* in a `.env` (plus the separate gitignore-leak check), so the user learns to rotate — only the destructive rewrite is gated. Proven end-to-end with the real CLI: the `.env` is left byte-for-byte unchanged while a key pasted into `CLAUDE.md` is still redacted. Regression cover in `ai_scan/mod.rs` and `ai_scan/redact.rs`; recorded as a security invariant.
